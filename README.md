...
Install a source extension, pick a series from the discovery home (trending, seasonal, and popular
rows pulled from AniList), and click **Download & track** to subscribe it. From that point
everything is automatic: new episodes are detected as they air, fetched through your enabled
extension, re-encoded with ffmpeg x265, and moved into your archive. You close the browser. It
keeps going.

**Features**

- **Extension-based sources.** ssanime-gui ships with no built-in sources. You install
  community-maintained source extensions (Hayase-compatible JS providers) from a repository URL,
  enable the ones you want, and the app routes searches through them. Add or remove sources at any
  time from the Extensions page.
...
- **Torrent-native, no external client.** An embedded `anacrolix/torrent` client downloads the
  magnets your extensions return; qBittorrent/Transmission backends are also supported if you
  prefer.
...
- **Feed auto-downloader.** Per-feed quality and regex filter rules. Interval-based polling runs in
...
- **Built-in DNS-over-HTTPS.** Resolves source hosts over DoH, transparently bypassing ISP-level
  DNS blocks — no VPN needed.
...
2. On first run, open **Extensions**, add a source repository by URL, and enable a source. The app
   ships with no sources of its own — this step tells it where to look.
3. Browse the discovery home — trending and seasonal rows load automatically from AniList.
4. Click a series you want, then **Download & track**. That's it.
5. The daemon searches your enabled extension, downloads, encodes, and archives each episode as it
   airs.
...
ssanime-gui is a media-management tool. It does not provide, host, index, or distribute any media
content, and it ships with no sources of its own. All sources are supplied by third-party
extensions that the user chooses to install and enable; the maintainers neither bundle nor endorse
any particular source or extension. You are solely responsible for the extensions you install and
for obtaining media through legal means in compliance with the laws of your jurisdiction.
