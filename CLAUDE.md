...
The source of truth for the data model, sourcing, and decisions is **`docs/reference/`**:
...
Key decisions baked into the build:
...
- **Sourcing: extensions-only** — every provider comes from a user-installed JS extension running in
  ...
  and repo index fetching (some ISPs DNS-block common trackers; DoH via `1.1.1.1` defeats it —
  verified). yt-dlp/direct/HLS still deferred behind the `Downloader` interface.
...
| `doh` | SSRF-guarded DNS-over-HTTPS resolver (bypasses ISP-level DNS blocks of source hosts) |
