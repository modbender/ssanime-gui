            ...
            <li class="flex items-start gap-4 px-5 py-4 hover:bg-white/[0.02] transition-colors duration-200">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1.5 flex-wrap">
                  ...
                  {#if p.is_builtin}
                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium bg-[rgb(var(--accent-rgb)/0.12)] text-[var(--color-accent)] ring-1 ring-[var(--color-accent)]/25">Built-in</span>
                  {/if}
                  {#if p.parent_id != null}
                    <span class="inline-flex items-center rounded-full bg-white/[0.06] px-2.5 py-0.5 text-[11px] font-medium text-[var(--color-text-dim)] ring-1 ring-white/10">
...
<Modal bind:open={editOpen} title={editMode === 'create' ? 'New profile' : 'Edit profile'}>
  ...
  <div class="space-y-5">
    <!-- Identity -->
    <fieldset class="space-y-4 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
      ...
      <div>
        ...
        <input id="prof-name" type="text" bind:value={form.name} placeholder="My custom profile" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
      </div>
      <div>
    ...
    <fieldset class="space-y-4 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            step="1"
            placeholder="inherit"
            class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)]/50 focus:outline-none focus:border-[var(--accent)] transition-colors"
          />
        </div>
        <div>
      ...
      <div>
        ...
        <input id="prof-resolutions" type="text" bind:value={form.output_resolutions} placeholder="1080, 720, 480" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
      </div>
    </fieldset>
    ...
    <fieldset class="space-y-4 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input id="prof-psy-rd" type="text" bind:value={form.psy_rd} placeholder="inherit" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
        </div>
        <div>
      ...
      <div>
        ...
        <input id="prof-deblock" type="text" bind:value={form.deblock} placeholder="inherit, e.g. -1,-1" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] font-mono transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
      </div>
      <div>
        ...
        <input id="prof-x265-params" type="text" bind:value={form.x265_params} placeholder="inherit" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] font-mono transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
      </div>
    </fieldset>

    <!-- Filters -->
    <fieldset class="space-y-3 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
      <legend class="px-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--color-muted)]">Filters</legend>
      <div class="flex gap-5 flex-wrap">
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" bind:checked={form.smartblur} class="rounded border-[var(--color-border-strong)] accent-[var(--accent)]" />
    ...
    <fieldset class="space-y-4 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input id="prof-audio" type="text" bind:value={form.audio} placeholder="copy / aac / flac" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
        </div>
        <div>
          ...
          <input id="prof-container" type="text" bind:value={form.container} placeholder="mkv / mp4" class="w-full h-9 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] transition-colors duration-200 focus:outline-none focus:border-[var(--accent)] focus:bg-[var(--color-surface-2)]" />
        </div>
      </div>
    </fieldset>
  </div>
...
<Modal bind:open={resolvedOpen} title="Resolved config — {resolvedName}">
  ...
  {:else if resolved}
    <div class="overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-2)]">
