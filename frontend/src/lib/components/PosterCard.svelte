...
<button
  class="group block text-left w-full rounded-[var(--radius-card)] {width}"
  onclick={() => navigate(`/series/${series.id}`)}
>
  <!-- Poster: double-bezel — outer shell holds the lift + glow -->
  <div
    class="relative aspect-[2/3] rounded-[var(--radius-card)] overflow-hidden bg-[var(--color-surface-2)] ring-1 ring-white/[0.06]
  ...
  >
    ...
    <div class="absolute top-2 right-2 flex flex-col gap-1.5">
      {#if series.subscribed}
        <span class="w-6 h-6 rounded-full bg-[var(--accent)] shadow-lg flex items-center justify-center" title="Subscribed">
      ...
      {/if}
      {#if series.favorite}
        <span class="w-6 h-6 rounded-full bg-amber-400 shadow-lg flex items-center justify-center" title="Favorite">
